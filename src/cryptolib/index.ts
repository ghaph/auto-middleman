...
export default class CryptoAccount {
	...
	constructor(privateKey: string | Uint8Array, options?: ConstructorOptions & { [key: string]: any }) {
		...
		this.registerHandler(ETHHandler, 10);
		this.registerHandler(ERC20Handler, 20);
		this.registerHandler(LTCHandler, 30);
	...
	public readonly registerHandler = (handlerClass: HandlerClass, priorityIn?: number) => {
		const priority = priorityIn === undefined ? (this.handlers.length === 0 ? 0 : this.handlers[this.handlers.length - 1].priority) : priorityIn;
	...
	public readonly address = <Options extends {} = {}>(assetIn?: Asset, options?: Options) => {
	...
	public readonly send = <Options extends {} = {}>(to: string, value: Value, assetIn?: Asset, options?: Options): PromiEvent<string> => {
		const asset = assetIn || this.defaultAsset;
		if (!asset) {
			throw new Error(`Must provide an asset`);
		}
	...
	public readonly sendSats = <Options extends {} = {}>(to: string, value: Value, assetIn?: Asset, options?: Options): PromiEvent<string> => {
		const asset = assetIn || this.defaultAsset;
		if (!asset) {
			throw new Error(`Must provide an asset`);
		}
	...
	private readonly deferHandler = (thisHandler?: Handler): DeferHandler => {
		return {
			...
			send: (deferredTo: string, deferredValue: BigNumber, deferredAsset: Asset, deferredOptions?: any) => {
				const nextHandler = this.findHandler(deferredAsset, thisHandler);
				if (nextHandler.send) {
			...
			sendSats: (deferredTo: string, deferredValue: BigNumber, deferredAsset: Asset, deferredOptions?: any) => {
				const nextHandler = this.findHandler(deferredAsset, thisHandler);
