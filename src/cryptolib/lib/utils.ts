 ...
 */
export const toHex = (array: Uint8Array): string => array.reduce((str, byte) => str + byte.toString(16).padStart(2, '0'), '');
 ...
 */
export const Ox = (hexInput: Uint8Array | string | number, { prefix } = { prefix: '0x' }): string => {
	let hexString: string = hexInput instanceof Uint8Array ? toHex(hexInput) : typeof hexInput === 'number' ? hexInput.toString(16) : hexInput;
...
export const shuffleArray = <T>(...arrayIn: T[] | T[][]): T[] => {
	const array: T[] = arrayIn.length === 1 && Array.isArray(arrayIn[0]) ? (arrayIn[0] as T[]) : (arrayIn as T[]);
