...
export const _apiFallbacks = {
	fetchUTXO: (testnet: boolean, txHash: string, vOut: number) => [
		...
		() => Blockchain.fetchUTXO(testnet ? BlockchainNetwork.BitcoinTestnet : BlockchainNetwork.Bitcoin)(txHash, vOut),
	],

	fetchUTXOs: (testnet: boolean, address: string, confirmations: number, scriptHash?: string) => [
		...shuffleArray(
		...
		() => Blockchain.fetchUTXOs(testnet ? BlockchainNetwork.BitcoinTestnet : BlockchainNetwork.Bitcoin)(address, confirmations),
		() => ElectrumX.fetchUTXOs('bitcoin', testnet)(address, confirmations, scriptHash),
	],

	fetchTXs: (testnet: boolean, address: string, confirmations: number = 0, scriptHash?: string) => [
		...shuffleArray(
			() => Blockstream.fetchTXs(testnet)(address),
			() => Blockchair.fetchTXs(testnet ? Blockchair.networks.BITCOIN_TESTNET : Blockchair.networks.BITCOIN)(address, confirmations),
			() => Blockchain.fetchUTXOs(testnet ? BlockchainNetwork.BitcoinTestnet : BlockchainNetwork.Bitcoin)(address, confirmations)
		),
	],

	broadcastTransaction: (testnet: boolean, hex: string) => [() => Blockchair.broadcastTransaction(testnet ? 'bitcoin/testnet' : 'bitcoin')(hex)],
};
...
export class BTCHandler implements Handler {
	...
	public readonly handlesAsset = (asset: Asset): boolean => typeof asset === 'string' && ['BTC', 'BITCOIN'].indexOf(asset.toUpperCase()) !== -1;
	...
	public readonly address = (asset: Asset, options?: AddressOptions): string => {
		const addr = bitcoin.payments.p2pkh({
			...
			network: this.testnet ? bitcoin.networks.testnet : bitcoin.networks.bitcoin,
		}).address;
		if (!addr) {
	...
	public readonly getBalance = async (asset: Asset, options?: BalanceOptions): Promise<BigNumber> =>
		(await this.getBalanceInSats(asset, options)).dividedBy(new BigNumber(10).exponentiatedBy(this.decimals));
	...
	// Transfer
	public readonly send = (to: string, value: BigNumber, asset: Asset, options?: TxOptions): PromiEvent<string> =>
		this.sendSats(to, value.times(new BigNumber(10).exponentiatedBy(this.decimals)), asset, options);
