...
const fetchTXs =
	(network: string) =>
	async (address: string, confirmations: number = 0, limit: number = 25): Promise<readonly UTXO[]> => {
		...
		for (let i = 0; i < Math.ceil(txHashes.length / 10); i++) {
			const txUrl = `https://api.blockchair.com/${network}/dashboards/transactions/${txHashes.slice(i * 10, (i + 1) * 10).join(',')}`;
			const txResponse = (
...
export const broadcastTransaction =
	...
	async (txHex: string): Promise<string> => {
		...
		const response = await axios.post<{ data: { transaction_hash: string } }>(url, { data: txHex }, { timeout: DEFAULT_TIMEOUT });
